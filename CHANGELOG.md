...
- `deploy`: a failing URL lookup fails the step. In `--output json` mode a failed command writes its diagnosis to stdout, which is a non-empty JSON document — so checking the output for emptiness let it through, published the diagnosis as the `urls` output and `null` as `url`, and called the deploy a success. The exit status is now what decides, and the diagnosis goes through `report_zad_error` like every other failure
- `deploy`: the `url` output of a multi-component deploy reports the first declared component that actually has an address, instead of `null` when the first one has no ingress. A `::notice::` names the component it fell back to, and a deploy where no component has an address is an error rather than an output nobody can use
