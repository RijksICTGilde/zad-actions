...
| `deployment-name`  | Yes      | -                          | Name of the deployment (e.g., `pr-85`)        |
| `component`        | Yes      | -                          | Component reference (e.g., `web`, `api`)      |
...
| `api-base-url`     | No       | ZAD Operations Manager URL | ZAD Operations Manager API base URL           |
| `comment-on-pr`    | No       | `false`                    | Post/update a PR comment with deployment URL  |
| `github-token`     | No       | `''`                       | GitHub token (`pull-requests: write`)         |
| `comment-header`   | No       | `## 🚀 Preview Deployment` | Custom header for PR comment                  |
